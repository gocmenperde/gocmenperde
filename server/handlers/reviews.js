...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const r = await pool.query(
      `SELECT id, name, rating, text, photos, verified_purchase, helpful_count, created_at
  ...
  if (req.method === 'POST') {
    ...
    if (!productId || !name || !text || !rating) return res.status(400).json({ error: 'productId, name, text ve rating zorunlu' });
    ...
    const ins = await pool.query(
      `INSERT INTO product_reviews(product_id,email_hash,name,rating,text,photos,verified_purchase,order_id,status,ip_hash)
       VALUES($1,$2,$3,$4,$5,$6::jsonb,$7,$8,$9,$10) RETURNING id`,
      [productId, email ? sha256(email) : null, name, rating, text, JSON.stringify(savedPhotos), verified, orderId, status, sha256(ip)]
    );
    return res.status(200).json({ success: true, id: ins.rows[0].id, status });
  }
